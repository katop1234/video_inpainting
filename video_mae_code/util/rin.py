...
class RINBlock(nn.Module):
    def __init__(
        ...
        **attn_kwargs
        ...
        self.latents_attend_to_patches = CrossAttention(dim_latent, dim_context = dim, norm = True, norm_context = True, **attn_kwargs)
        ...
        for _ in range(latent_self_attn_depth):
            self.latent_self_attns.append(nn.ModuleList([
                CrossAttention(dim_latent, norm = True, **attn_kwargs),
        ...
        self.patches_self_attn = SelfAttention(dim, norm = True, **attn_kwargs)
        self.patches_self_attn_ff = FeedForward(dim)

        self.patches_attend_to_latents = CrossAttention(dim, dim_context = dim_latent, norm = True, norm_context = True, **attn_kwargs)
...
class RIN(nn.Module):
    ...
    def forward(
        ...
        # False by default, we are not concerned with time
