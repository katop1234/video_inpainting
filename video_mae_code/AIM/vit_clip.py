...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, mlp_ratio=4., attn_mask: torch.Tensor = None, scale=1., num_tadapter=1, num_frames=8, drop_path=0.):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, T=16):
        ...
        if self.num_tadapter == 2:
            ...
            xt = self.T_Adapter(self.attention(self.T_Adapter_in(self.norm1(xt))))
        else:
            # xt = self.T_Adapter(self.attention(self.ln_1(xt)))
            xt = self.T_Adapter(self.attention(self.norm1(xt)))
        xt = rearrange(xt, 't (b n) d -> n (b t) d', n=n)
        x = x + self.drop_path(xt)
        ## spatial adaptation
        # x = x + self.S_Adapter(self.attention(self.ln_1(x)))
        x = x + self.S_Adapter(self.attention(self.norm1(x)))
        ## joint adaptation
        # xn = self.ln_2(x)
        xn = self.norm2(x)
